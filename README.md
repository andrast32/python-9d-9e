Matkul basic python coding

Senin - 19 Januari 2026
introduction to python coding -> perkenalan -> installasi python -> ide untuk coding python (vs code) -> sintaks dasar python (perintah print, komentar, variabel, tipe data dasar) latihan sederhana

Senin - 26 Januari 2026
Aritmatika dan Operasi Dasar -> operator aritmatika (penjumlahan, pengurangan, perkalian, pembagian) -> latihan membuat kalkulator BMI sederhana

Senin - 2 Februari 2026
Pengkondisian -> memahami konsep percabangan (if, elif, else) -> menggunakan percabangan untuk mengklasifikasikan data -> membuat program sederhana untuk menentukan kategori umur -> operator perbandingan ==, !=, >, <, >=, <= -> operator logika and, or, not

senin - 20 April 2026
fungsi def -> memahami konsep fungsi -> membuat fungsi sederhana untuk menghitung luas dan keliling lingkaran -> penggunaan parameter dan return value